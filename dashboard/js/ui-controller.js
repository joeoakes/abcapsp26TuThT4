...
function startTelemetryLoop(mission) {
  ...
  state.telemetryFeedTimer = setInterval(() => {
    ...
    if (state.telemetryQueue.length > 100) {
  ...
  const renderQueue = () => {
    if (state.telemetryQueue.length) {
      ...
      stream.prepend(line);
      while (stream.children.length > 16) {
...
function updateHudBars(ts) {
  if (!state.hudLastTs || ts - state.hudLastTs > 180) {
    const drift = (value, delta, min, max) => Math.max(min, Math.min(max, value + (Math.random() * delta * 2 - delta)));
    state.hudValues.cpu = drift(state.hudValues.cpu, 5, 8, 94);
    state.hudValues.ram = drift(state.hudValues.ram, 4, 10, 95);
    state.hudValues.cache = drift(state.hudValues.cache, 7, 6, 92);
...
function startLatencyTicker() {
  ...
  state.latencyTimer = setInterval(() => {
    const latency = (9 + Math.random() * 8).toFixed(0);
